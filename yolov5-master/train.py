...
D:\\Tools-data\\PyCharm\\Object-detection-based-on-Two
Usage:
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    from MoCo.main import moco_train
    from MoCo.main import moco_train_loader
    from MoCo.main import moco_model
    from MoCo.main import moco_model_1
    from MoCo.main import moco_optimizer
    from MoCo.main import moco_args


    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        moco_loss = moco_train(moco_model, moco_model_1, moco_train_loader, moco_optimizer, epoch, moco_args)
        moco_loss = moco_loss.view(1).detach()
        model.train()
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ni = i + nb * epoch  # number integrated batches (since train start)集成的批次数量（从训练开始）
            ...
            # Forward
            with amp.autocast(enabled=cuda):        # 修改处 enabled=cuda
                pred = model(imgs)  # forward
                yolov5_loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size

                loss = yolov5_loss + 0.1*moco_loss
        ...
        # print("yolov5_loss = " + str(yolov5_loss))
        # print("moco_loss = " + str(moco_loss))
        # print("loss = " + str(loss))
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='my_data_v/mydata_1.yaml', help='model.yaml path')#修改处

    parser.add_argument('--data', type=str, default='my_data_v/mydata.yaml', help='dataset.yaml path')#修改处
    parser.add_argument('--hyp', type=str, default='data/hyps/hyp.scratch.yaml', help='hyperparameters path')

    parser.add_argument('--epochs', type=int, default=100)#修改处，显卡低的可以调到100，原来是300
