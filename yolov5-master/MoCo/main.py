...
if args.results_dir == '':
    args.results_dir = "D:\\Tools-data\\PyCharm\\Object-detection-based-on-Two\\2_Contrast_Learning_For_Object_Detection\\yolov5-master\\MoCo\\run\\cache-" + datetime.now().strftime("%Y-%m-%d-%H-%M-%S-moco")
...
train_data = CIFAR10Pair(root="D:\\Tools-data\\PyCharm\\Object-detection-based-on-Two\\2_Contrast_Learning_For_Object_Detection\\yolov5-master\\MoCo\\data_v", train=True, transform=train_transform, download=False)
...
memory_data = CIFAR10(root="D:\\Tools-data\\PyCharm\\Object-detection-based-on-Two\\2_Contrast_Learning_For_Object_Detection\\yolov5-master\\MoCo\\data_v", train=True, transform=test_transform, download=False)
memory_loader = DataLoader(memory_data, batch_size=args.batch_size, shuffle=False, num_workers=0, pin_memory=True)

test_data = CIFAR10(root="D:\\Tools-data\\PyCharm\\Object-detection-based-on-Two\\2_Contrast_Learning_For_Object_Detection\\yolov5-master\\MoCo\\data_v", train=False, transform=test_transform, download=False)
...
def moco_train(net, net_1, data_loader, train_optimizer, epoch, args):
    ...
    return loss_add / total_num       #  yolov5需要的损失
...
# epoch_start = 1
# if args.resume is not '':           # 加载预模型
#     checkpoint = torch.load(args.resume)
#     moco_model.load_state_dict(checkpoint['state_dict'])
#     moco_optimizer.load_state_dict(checkpoint['optimizer'])
...
#    train_loss = moco_train(moco_model, moco_train_loader, moco_optimizer, epoch, args)
#    results['train_loss'].append(train_loss)
#    test_acc_1 = test(moco_model.encoder_q, memory_loader, test_loader, epoch, args)
...
#
#    # save model
#    torch.save({'epoch': epoch, 'state_dict': moco_model.state_dict(), 'optimizer': moco_optimizer.state_dict(), },
