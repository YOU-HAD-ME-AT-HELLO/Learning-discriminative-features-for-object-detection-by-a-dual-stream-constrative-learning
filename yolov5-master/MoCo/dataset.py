...
figure_path = './data/train/33'  # 图片的位置
figure_name_label = './data_processing/figure_name_label_train/image_train_33_list.txt'  # 保存图片名称和标签
batch_save = './data_processing/batch_save_train'  # 保存batch文件
...
def read_data(filename, data_path, shape=None, color='RGB'):
    """
       filename (str): a file
         data_processing file is stored in such format:
           image_name  label
       data_path (str): image data_processing folder
       return (numpy): a array of image and a array of label
    """
    (shape1, shape2) = shape
    if os.path.isdir(filename):
        print("Can't found data_processing file!")
    else:
        f = open(filename)
...
def pickled(savepath, data, label, fnames, bin_num=BIN_COUNTS, mode="train", name=None):
    '''
      ...
      data_processing (array): image data_processing, a nx3072 array
      label (list): image label, a list with length n
      fnames (str list): image names, a list with length n
      bin_num (int): save data_processing in several files
      mode (str): {'train', 'test'}
    '''
    ...
    for i in range(bin_num):
        ...
        if end <= total_num:
            dict = {'data_processing': data[start:end, :],
                    'labels': label[start:end],
                    'filenames': fnames[start:end]}
        else:
            dict = {'data_processing': data[start:, :],
                    'labels': label[start:],
...
if __name__ == '__main__':
    ...
    # py2bin(data_processing, label) #将像素矩阵和标签列表转成cifar10 binary version # 二进制版本
    pickled(save_path, data, label, lst, bin_num=1, name='2')  # 生成python版本
